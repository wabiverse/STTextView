...
extension ViewController: STTextViewDelegate
{
  func textView(_ textView: STTextView, didChangeTextIn affectedCharRange: NSTextRange, replacementString: String)
  {
    ...
    for annotation in annotations where textView.textContentManager.offset(from: affectedCharRange.endLocation, to: annotation.location) >= 0
    {
...
{
  func textViewAnnotations(_: STTextView) -> [STLineAnnotation]
  {
    annotations
  }
  ...
  {
    guard let myLineAnnotation = lineAnnotation as? LineAnnotation
    else
    {
      return nil
    }
