...
extension STTextView
{
  ...
  @objc private func _dragSelectedTextGestureRecognizer(gestureRecognizer: NSGestureRecognizer)
  {
    ...
    guard !currentSelectionRanges.isEmpty
    else
    {
      return
    }

    lazy var interactionInSelectedRange: Bool = currentSelectionRanges.reduce(true)
    { partialResult, range in
      guard let interationLocation = textLayoutManager.location(interactingAt: eventPoint, inContainerAt: range.location)
      else
      {
    ...
    guard interactionInSelectedRange, let selectionsAttributedString = textLayoutManager.textSelectionsAttributedString(), let textRange = currentSelectionRanges.first
    else
    {
      return
    }
